    ...
    async callback(event, trigger, player) {
      ...
      if (result.bool && result.opinion) {
        if (result.opinion == "red") {
          ...
          const resultx = await player.chooseTarget(prompt, function(card, player2, target) {
            const bool1 = (current) => !get.event().targets1.includes(current);
            return bool1(target);
          }).set("complexTarget", true).set("targets1", result.targets).set("ai", (target) => {
          ...
          for (let color of result.opinions) {
            if (result[color]?.length) {
              cards2.addArray(result[color].map((i) => i[1]));
